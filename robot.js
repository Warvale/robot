...
client.on(`message`, (msg) => {
    ...
    if (msg.content,toLowerCase().startsWith(prefix + `ban`)) {
        exports.run = (client, message, args) => {
            ...
            let modlog = client.channels.find('name', 'warvale-bot-logs');
            if (reason.length < 1) return message.reply('Please input a reason.');
            if (message.mentions.users.size < 1) return message.reply('Please mention someone to ban them.').catch(console.error); 
            
            if (!message.guild.member(user).bannable) return message.reply('I do not have permission to ban that member.');
